...
| **R2** | **Apple-MapKit-Nutzungsbedingungen.** POI-Ergebnisse aus `MKLocalSearch` dürfen nicht dauerhaft in einer eigenen Datenbank gespeichert und außerhalb einer Apple-Karte weiterverwendet werden. | 🟠 | POI-Suche nur als **Eingabehilfe**: Der Nutzer bestätigt Name und Position, wir legen eine **eigene** Entität mit eigener ID an. Keine Apple-Identifier, keine Adressdaten, keine Importe. **Vor App-Store-Einreichung rechtlich prüfen lassen.** | P0.4 / vor P1 |
| **R3** | **Supabase pausiert kostenlose Projekte nach 7 Tagen Inaktivität.** Der Tester öffnet die App und sie funktioniert nicht. | 🟠 | GitHub-Actions-Workflow pingt alle 3 Tage einen leichten Endpunkt. 0 €, ~15 Zeilen YAML. | P0.1 |
...
| **R8** | **Free-Tier-Konditionen ändern sich.** Alle Angaben stammen von Mai 2026. | 🟡 | Verifikation als Aufgabe in P0.1. Ausstiegsplan in `04-cost-analysis.md` §5: vollständiger Umzug in 3–5 Tagen, weil es Postgres ist. | P0.1 |
